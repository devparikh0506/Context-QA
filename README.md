...
Context QA is a scalable RAG-based question-answering system. The goal of this repository is to create a scalable project based on RAG operations of a vector database (Postgres with pgvector), and to expose a question-answering system developed with LangChain and FastAPI on a Next.js frontend.
...
1. **Environment**
   - Copy `.env.example` to `.env` and set DB URL, `SECRET_KEY_ACCESS_API`, `OPENAI_API_KEY`, `OPENAI_ORGANIZATION`, `FIRST_SUPERUSER`, `FIRST_SUPERUSER_PASSWORD`, etc.
...
3. **Database**
   - Start **Cloud SQL Proxy** (if using Cloud SQL) or ensure Postgres with pgvector is reachable at the host/port in `.env`.
...
5. **Start the API**
   ...
   - Get a token: `POST /api/v1/login/access-token` (form: `username`, `password` = your `FIRST_SUPERUSER` / `FIRST_SUPERUSER_PASSWORD`).

For full setup (Terraform, Cloud Build, frontend), see the sections below and [docs/getting-started/installation.md](docs/getting-started/installation.md).
...
[Adding a Submodule and Committing Changes: Git, Terraform, FastAPI (external guide)](https://medium.com/@saverio3107/adding-a-submodule-and-committing-changes-git-terraform-fastapi-6fe9cf7c9ba7?sk=595dafdaa36427a2d6efee8c08940ee9)
...
Maintained by **Dev Parikh**. Contributions are welcome! For major changes, please open an issue first to discuss what you would like to change.

## License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details. Copyright (c) Dev Parikh.
